...
func New(apiKey, model string, httpClient *http.Client) *Client {
	...
	return &Client{APIKey: apiKey, Model: model, BaseURL: defaultBaseURL, HTTP: httpClient}
}
...
func (c *Client) Assess(ctx context.Context, assignment string, sources []domain.SourceEvidence, history []domain.StoryHistory) (domain.Assessment, error) {
	...
	if err := c.complete(ctx, "newsroom_assessment", prompt, schema, &output); err != nil {
		return output, err
	}
...
func (c *Client) Draft(ctx context.Context, assignment string, sources []domain.SourceEvidence, history []domain.StoryHistory) (domain.FinalOutput, error) {
	...
	if err := c.complete(ctx, "newsroom_drafts", prompt, schema, &output); err != nil {
		return output, err
	}
	return output, nil
}

func (c *Client) complete(ctx context.Context, name, prompt string, schema map[string]any, target any) error {
	if strings.TrimSpace(c.APIKey) == "" || strings.TrimSpace(c.Model) == "" {
		return fmt.Errorf("OpenRouter API key and model are required")
	}
	body := map[string]any{"model": c.Model, "messages": []map[string]string{{"role": "user", "content": prompt}}, "temperature": 0.2, "max_tokens": 1800, "provider": map[string]any{"require_parameters": true}, "response_format": map[string]any{"type": "json_schema", "json_schema": map[string]any{"name": name, "strict": true, "schema": schema}}}
		...
		if err == nil {
			...
			if resp.StatusCode >= 200 && resp.StatusCode < 300 {
				...
				if len(envelope.Choices) != 1 || strings.TrimSpace(envelope.Choices[0].Message.Content) == "" {
					return fmt.Errorf("OpenRouter response has no structured content")
				}
